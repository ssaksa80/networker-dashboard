...
<body class="connection-open">
    ...
    <section class="dashboard">
      ...
      <section class="section" aria-label="Local snapshot growth">
        ...
        <div class="snapshot-controls">
          <label>
            Compare range
            <select id="snapshotRange">
              <option value="7d" selected>Last 7 days</option>
              <option value="30d">Last 30 days</option>
              <option value="90d">Last 90 days</option>
            </select>
          </label>
          <button id="snapshotSaveBtn" class="primary" type="button">Save snapshot</button>
  ...
  <script>
    ...
    function snapshotColor(value) {
      const numeric = Number(value);
      if (!Number.isFinite(numeric) || numeric === 0) return "var(--ink)";
      return numeric > 0 ? "var(--red)" : "var(--green)";
    }
    ...
    function renderSnapshotComparison(data) {
      ...
      if (!data.ok || !Array.isArray(data.metrics) || !data.metrics.length) {
        snapshotGrid.innerHTML = `
          <div class="snapshot-cell">
            <span>Snapshot status</span>
            <strong>${escapeHtml(data.ok ? "No comparison" : "Unavailable")}</strong>
            <small>${escapeHtml(data.message || "Save at least two snapshots to compare growth.")}</small>
          </div>`;
        return;
      }
      snapshotGrid.innerHTML = data.metrics.map((item) => {
        const delta = Number(item.delta || 0);
        const sign = delta > 0 ? "+" : "";
        const percent = item.deltaPercent === null || item.deltaPercent === undefined ? "--" : `${sign}${formatDecimal(item.deltaPercent)}%`;
        return `
          <div class="snapshot-cell">
            <span>${escapeHtml(item.label)}</span>
            <strong style="color:${snapshotColor(delta)}">${sign}${formatNumber(delta)}</strong>
            <small>${escapeHtml(item.previousDate || data.previousDate || "--")}: ${formatNumber(item.previous)} &rarr; ${escapeHtml(item.currentDate || data.currentDate || "--")}: ${formatNumber(item.current)} (${percent})</small>
          </div>`;
      }).join("");
    }
    ...
    async function compareLocalSnapshots() {
      ...
      try {
        const response = await fetch(`/api/snapshots?range=${encodeURIComponent(snapshotRange.value)}`, {cache: "no-store"});
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || `Snapshot compare failed with HTTP ${response.status}`);
    ...
    document.addEventListener("keydown", (event) => {
      if (event.key === "Escape" && alertAutomationModal.classList.contains("open")) {
        closeAlertAutomationModal();
      }
    });
    ...
    snapshotCompareBtn.addEventListener("click", () => {
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        if not self._require_https():
            return
        path = urlparse(self.path).path
        if path not in ("/api/dashboard", "/api/export", "/api/server-health", "/api/alert-automation", "/api/snapshots"):
            self._send_error_json(HTTPStatus.NOT_FOUND, "Not found.")
            return
